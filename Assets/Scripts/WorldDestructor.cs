...
public class WorldDestructor : MonoBehaviour
{
    ...
    List<BlockSpawned> blocks = new List<BlockSpawned>();
    ...
    private void Noob_Collisioned(CollisionHandler part, Collision2D coll)
    {
        ...
        if (tilemap)
        {
            ...
            Vector2 center = new Vector2(contactPos.x, contactPos.y);
                    ...
                    if(distance < radius)
                    {
                        ...
                        if (tilemap.transform.position != tilemapPos)
                        {
                            try
                            {
                                ...
                                var otherPos = new Vector3Int(zaebalx + Mathf.RoundToInt(offset.x), zaebaly + Mathf.RoundToInt(offset.y), 0);
                        ...
                        {
                            var pos = new Vector3Int(zaebalx, zaebaly, 0);
                    ...
                    if (distance < radius)
                    {
                        ...
                        if (tilemap.transform.position != tilemapPos)
                        {
                            try
                            {
                                ...
                                var otherTilemap = generator.dict[intPos].Tilemap;
                                var offset = tilemap.transform.position - tilemapPos;
                                var otherPos = new Vector3Int(zaebalx + (int)offset.x, zaebaly + (int)offset.y, 0);
